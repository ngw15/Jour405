...
```{r}
soccer<-read_csv("https://raw.githubusercontent.com/dwillis/NCAAWomensSoccerData/refs/heads/main/data/ncaa_womens_soccer_matchstats_2024.csv")
```
### Task 3: Create a new column called `goal_diff` that calculates the difference between the two teams' goals.
```{r}
soccer <- soccer%>%mutate(goal_diff = team_score - opponent_score)
```
...
```{r}
soccer %>% summarize(correlation = cor(fouls, goal_diff, method="pearson"))
```
...
Correlation values that are close to 1 are an indicator of a strong relationship. The correlation between foals and goal differentials is 0.01806104	which is really far from 1 and a lot closer to 0, so we can say that there is no relationship between the two. The scatterplot is all over the place, which also doesn't indicate any relationship between the two. 
