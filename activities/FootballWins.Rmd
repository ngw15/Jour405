...
No, I don't think so, because only 62.32% of the variability in wins is explained by the model, which isn't that high, meaning we can't explain the variability in wins. Along with that only 58.28% of the variance is explained by the model. 
...
Points scored is the most impactful on wins while yards gained doesn't seem to have an impact at all. With the yards gained the coefficient was so small and negative that it had a small negative impact on yeards gained. For points scored the coefficient was significant because it showed that each additional point scored is associated with a small increase in wins. 
...
We can use this model to cover a sports game by analyzing how they did in regular games. If we're trying to predict whether an NFL team will win a playoff game we can look at the previous games and practice games to see how the team normally scored. We can see how the offense and defense performs based on the data. We can find the numbers that show the impact of the yards gained, scored points and the takeaways to see what contributes most to the teams winning scores. 
