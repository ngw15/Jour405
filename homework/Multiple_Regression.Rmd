...
There doesn't seem to be any that are correlated, maybe with income and the percent of the population to have a bachelors degree. It looks like the lower the income, the less people have a degree. But a lot doesn't look correlated at all, like turnout and population don't seem to have any connection, or with income or age. There might be a linear relationship between turnout and the percent of people who are unemployed. 
...
The range of the model with the minimum and maximum are all kind of extreme. With all of the variables, there are really low minimum outliers that don't really fit the trend of data. I think most of the data points are centered around the models. The data isn't really clustered around the model but it is centered around it. The relationship between turnout and income, median age and the amount of people who have a bachelors degree are all positive. I don't think the population and turnout relationship is significant. Higher income does correlate to higher turnout to an extent, it looks like there are other influences that may be causing higher turnout. Education also has a slight influence on turnout. There seems to be a semi strong relationship between unemployment and voter turnout, showing that the more unemployed people there are, the less people go to vote. Age doesn't seem to impact turnout at all and race does seem to play a part in voter turnout. 
...
The percent of white people with voter turnout seems to be significantly better than the others because the R-squared variable is the lowest. 
...
# Model 4
model4 <- lm(turnout ~ income, data = md_turnout)
summary(model4)
...
This type of analysis can be used to understand how income level, education, age, race, employment are linked to voter turnout. This can be used to predict which canidate would win for a story and the probablities of that candidate winning. We can see which factors have the biggest impact on unusual voting patterns. 


In a story, this kind of analysis lets you go beyond surface-level trends and dig into the deeper social forces influencing election outcomes. It gives your reporting more depth and helps your audience understand the "why" behind the numbers.
